...
import { AQUA, GRAY, WHITE } from "../../exports";
import { EntityArmorStand } from "../../exports";
...
register('tick', () => {
    if (settings.slayerKillTime) {
        World.getAllEntitiesOfType(EntityArmorStand).forEach(stand => {
            ...
            if (name.includes(' 0❤') && (name.includes('Horror') || name.includes('Packmaster') || name.includes('Broodfather') || name.includes('Seraph') || name.includes('Demonlord')) && killTime == undefined && Math.hypot(stand.getX() - bossX, stand.getY() - bossY, stand.getZ() - bossZ) < 1) {
