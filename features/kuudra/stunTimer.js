...
register("chat", (message) => {
    if (settings.stunTimer) {
        ...
        if (settings.entryTimer && message == "[NPC] Elle: Oh, funny seeing you here.") ChatLib.chat(`${GRAY}[${AQUA}TurtleAddons${GRAY}] ${WHITE}Entry took ${((new Date().getTime() - p3) / 1000).toFixed(2)}s.`);
    };
}).setCriteria("${message}");
