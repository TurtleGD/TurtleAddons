...
register("tick", () => {
    ...
    if (kuudraLastHP > kuudra.getEntity()?.func_110143_aJ() && kuudraLastHP - kuudra.getEntity()?.func_110143_aJ() > threshold / 9600 && kuudraLastHP - kuudra.getEntity()?.func_110143_aJ() < 26_000 && inTrueLair()) ChatLib.chat(`Someone pulled for ${GREEN}${formatNumber(((kuudraLastHP - kuudra.getEntity()?.func_110143_aJ()) * 9600).toFixed(0))} ${RESET}damage at ${GREEN}${((new Date().getTime() - p4Teleport) / 1000).toFixed(2)}s/${((new Date().getTime() - p4Start) / 1000).toFixed(2)}s${RESET}.`)
