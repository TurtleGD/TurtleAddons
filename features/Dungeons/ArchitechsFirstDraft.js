...
register("chat", (message) => {
    if (settings.architectsFirstDraft) {
        if (message.startsWith("PUZZLE FAIL!") || message == "[STATUE] Oruo the Omniscient: Yikes") {
            Client.scheduleTask(1, () => ChatLib.chat(new TextComponent(`${GRAY}[${AQUA}TurtleAddons${GRAY}]${RESET} Click here to get an Architect's First Draft!`).setClick('run_command', "/gfs architect's first draft 1")));
        }
    }
}).setCriteria("${message}");
