...
register('command', () => {
    World.getAllEntitiesOfType(EntityArmorStand).forEach(stand => {
        let helmetHelmet
        if (Player.asPlayerMP().distanceTo(stand.getX(), Math.round(stand.getY()), Math.round(stand.getZ())) < 3) helmetHelmet = new EntityLivingBase(stand.getEntity())?.getItemInSlot(4)?.getRawNBT()
        if (helmetHelmet) console.log(helmetHelmet)
    })
}).setName('test')
