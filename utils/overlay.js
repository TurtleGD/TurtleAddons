...
import { AQUA, BOLD, GOLD, GREEN, RED, RESET } from "./formatting";
...
register("step", () => {
    if (!gui.isOpen()) {
        ...
        if (settings.goldorTickTimer) overlays.push(["goldorTickTimer", pogData.goldorTickTimerX, pogData.goldorTickTimerY, pogData.goldorTickTimerScale, `${GREEN}3.00s`]);
        if (settings.smolderingPolarizationTimer) overlays.push(["smolderingPolarizationTimer", pogData.smolderingPolarizationTimerX, pogData.smolderingPolarizationTimerY, pogData.smolderingPolarizationTimerScale, `${GREEN + BOLD}Smoldering Polarization:`]);
    }
}).setDelay(1)
